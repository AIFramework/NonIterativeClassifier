...
namespace NonIterativeTest
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            VectorClass[] dataset = GetIrisDataset(); // Получение датасета
            ...
            VectorClass[] test = new VectorClass[len75];
            VectorClass[] train = new VectorClass[len25];
            for (int i = 0; i < len75; i++)
            {
                test[i] = dataset[i];
            }
        ...
        private static VectorClass[] GetIrisDataset(string path = "iris.txt")
        {
            string[] objects = File.ReadAllLines(path);
            List<VectorClass> dataset = new List<VectorClass>(objects.Length);
            System.Globalization.NumberFormatInfo provider = AISetting.GetProvider();
            ...
            foreach (string item in objects)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    features[i] = Convert.ToDouble(strs[i], provider);
                }
        ...
        private static void Test(VectorClass[] test)
        {
